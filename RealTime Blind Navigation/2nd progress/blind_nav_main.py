...
from yolo_utils import YoloPostProcessing, HostCollisionTracker, draw_birds_eye_view
from audio_announcer import SpatialAudioAnnouncer, speak
...
def create_pipeline():
    ...
    # 1. Color Camera Configuration
    cam = pipeline.create(dai.node.ColorCamera)
    cam.setResolution(dai.ColorCameraProperties.SensorResolution.THE_12_MP)
    cam.setIspScale(1, 3)          # Downscale 12MP sensor to 1344x1008
...
def main():
    print("\n🟢 Starting Predictive Blind Navigation - Phase 3/4...")
    ...
    with dai.Device(create_pipeline()) as device:
        ...
        # Retrieve factory-calibrated lens intrinsics
        try:
            calibData = device.readCalibration()
            # Reference CAM_A (color) scaled to match our ISP frame size (1344x1008)
            intrinsics = calibData.getCameraIntrinsics(dai.CameraBoardSocket.CAM_A, 1344, 1008)
            fx, fy, cx, cy = intrinsics[0][0], intrinsics[1][1], intrinsics[0][2], intrinsics[1][2]
            print(f"ℹ️ Loaded intrinsics: fx={fx:.1f}, fy={fy:.1f}, cx={cx:.1f}, cy={cy:.1f}")
        except Exception as calib_err:
            # Fallback values if extraction fails
            fx, fy, cx, cy = 1012.0, 1012.0, 672.0, 504.0
            print(f"⚠️ Calibration failed. Loading default factory assumptions: {calib_err}")
        
        # Instantiate Host Collision Tracker
        ...
        with group.activate():
            with InferVStreams(group, in_params, out_params) as pipe:
                ...
                while True:
                    ...
                    # Fuse detections with depth metrics
                    output_image, objects = yolo_parser.process_and_draw(raw_res, f_bgr, depth_map=f_dep)

                    # Update Host Trackers and analyze trajectories
                    ...
                    # Pass alerts to announcer update
                    ...
                    # Draw text alerts for active dangerous collision paths
                    for track_id, alert in alerts.items():
                        if alert["is_dangerous"]:
                            ...
                            cv2.putText(
                                output_image, 
                                f"COLLISION ID {track_id}: {lbl} ({spd:.1f}m/s, TTI:{tti:.1f}s)", 
                                (20, 80 + int(track_id)*30), 
                    ...
                    # Overlay the 250x250 mini-map directly into the top-right corner of the feed
