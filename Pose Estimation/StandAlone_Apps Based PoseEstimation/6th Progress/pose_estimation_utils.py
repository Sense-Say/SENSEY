...
class PoseEstPostProcessing:
    ...
    def visualize_pose_estimation_result(
            ...
            model_height: int,
            model_width: int,
            depth_map: np.ndarray = None, # NEW: Passed from OAK-D Lite
            detection_threshold: float = 0.5,
            joint_threshold: float = 0.5,
    ) -> np.ndarray:
        
        import time, cv2, json, os, sys, threading, subprocess, numpy as np

        # --- 1. INITIALIZATION & FAIL-SAFE SETUP ---
        ...
        # --- 2. DATA EXTRACTION ---
        try:
            if 'predictions' in results: b_data = results['predictions'][0]; s_data = results['predictions'][1]; k_data = results['predictions'][2]; ks_data = results['predictions'][3]
            elif 'bboxes' in results: b_data = results['bboxes'][0]; s_data = results['scores'][0]; k_data = results['keypoints'][0]; ks_data = results['joint_scores'][0]
            else: return image
        except: return image
        ...
        # --- 4. DRAWING LOOP & DATA COLLECTION ---
        action_groups = {} 
        ...
        for i, (box, score, kp, kp_score) in enumerate(zip(b_data, s_data, k_data, ks_data)):
            ...
            det_box = self.map_box_to_original_coords(box, orig_w, orig_h, model_width, model_height)
            xmin, ymin, xmax, ymax = [int(x) for x in det_box]

            # --- SPATIAL Z-DEPTH LOGIC ---
            distance_m = 0.0
            cx, cy = (xmin + xmax) // 2, (ymin + ymax) // 2
            if depth_map is not None:
                # Clamp coordinates to depth map size
                cx_depth = min(max(0, cx), depth_map.shape[1] - 1)
                cy_depth = min(max(0, cy), depth_map.shape[0] - 1)
                z_mm = depth_map[cy_depth, cx_depth]
                distance_m = z_mm / 1000.0 # Convert to Meters
            # -----------------------------

            # Action Logic
            raw_kp = kp.reshape(17, 2)
            fmt_kp = [[p[0], p[1], s] for p, s in zip(raw_kp, kp_score)]
            act, col = self.action_monitor.get_action(fmt_kp, i, (cx, cy))

            name = self.name_map.get(str(i), f"Student {i+1}")
            dist_label = f"{distance_m:.1f}m" if distance_m > 0 else ""
            
            # Data for Audio (e.g., "Edward at 2 meters")
            audio_name = f"{name} at {distance_m:.1f} meters" if distance_m > 0 else name
            ...
            cv2.rectangle(image, (xmin, ymin-25), (xmin+280, ymin), col, -1)
            cv2.putText(image, label_text, (xmin+5, ymin-7), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1)
            
            # Skeleton
            mapped_kp = self.map_keypoints_to_original_coords(raw_kp, orig_w, orig_h, model_width, model_height)
            for j, s in zip(mapped_kp, kp_score):
