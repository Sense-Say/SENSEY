...
class PoseEstPostProcessing:
    ...
    def visualize_pose_estimation_result(
        ...
        for (detection_box, detection_score, detection_keypoints,
            ...
            # Convert detection_score to a float to prevent numpy formatting errors
            score_val = float(detection_score)
            ...
            # --- THE FIXED LINE ---
            label = f"Person {person_idx}: {score_val:.2f}"
            
            cv2.rectangle(image, (xmin, ymin), (xmax, ymax), (255, 0, 0), 2)
            cv2.rectangle(image, (xmin, ymin - 25), (xmin + 160, ymin), (255, 0, 0), -1)
            cv2.putText(image, label, (xmin + 5, ymin - 7), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            ...
            for joint, joint_score in zip(detection_keypoints, detection_keypoints_score):
                if joint_score < joint_threshold:
                    continue
                cv2.circle(image, (int(joint[0]), int(joint[1])), 2, (255, 0, 255), -1)
