...
class PoseEstPostProcessing:
    ...
    def visualize_pose_estimation_result(
        ...
        if self.snap_button and self.snap_button.is_pressed:
            button_pressed = True
        ...
        if key == ord('s') or button_pressed:
            ...
            print("🛑 TRIGGER: Exiting Pose to run Face Scan...")
            try: os._exit(0)
            except: sys.exit(0)
        ...
        # --- 5. SMART REPORTING (Once after startup) ---
        if not self.report_printed and len(action_groups) > 0:
            ...
            for action, names_list in action_groups.items():
                ...
                # Split Real Names vs. Generic Students
                real_names = []
                student_count = 0
                for n in names_list:
                    if "Student" in n: student_count += 1
                    else: real_names.append(n)
                
                # Build Logic String
                parts = []
                if len(real_names) > 0: parts.append(", ".join(real_names))
            ...
            if AUDIO_ENABLED:
                def speak_report():
                    try:
                        ...
                        audio_file = "/home/raspberrypi/Documents/report.mp3"
                        tts.save(audio_file)
                        print("🔊 Speaking...")
                ...
                threading.Thread(target=speak_report, daemon=True).start()
