...
def tts_worker():
    ...
    # Active listening loop (Warmup sequence removed for USB Bluetooth stability)
    while True:
        ...
        try:
            piper_cmd = (
                ...
                f"{PIPER_EXE} --model {PIPER_MODEL} --length_scale 1.00 --output_file {ram_file}"
            )
...
class PoseEstPostProcessing:
    def __init__(self, max_detections=15, score_threshold=0.3, nms_iou_thresh=0.45, regression_length=15, strides=[8, 16, 32]):
        ...
        # --- CONTINUOUS TRACKING STATES ---
        self.tracks = {}  # track_id -> {centroid, posture, action, score, box, name, color}
        self.next_track_id = 0
        self.stability_threshold = 15  # Consecutive frames required to shift stable states
    ...
    def visualize_pose_estimation_result(self, results, image, model_height, model_width, depth_map=None, intrinsics=None, vpu_faces=None, detection_threshold=0.4, joint_threshold=0.4, key_pressed=255):
        ...
        try:
            if 'predictions' in results: 
                b_d, s_d, k_d, ks_d = results['predictions'][0], results['predictions'][1], results['predictions'][2], results['predictions'][3]
            elif 'bboxes' in results: 
                b_d, s_d, k_d, ks_d = results['bboxes'][0], results['scores'][0], results['keypoints'][0], results['joint_scores'][0]
            else: 
        ...
        for i, (box, score, kp, kp_score) in enumerate(zip(b_d, s_d, k_d, ks_d)):
            if float(score) < detection_threshold or np.isnan(box).any(): 
                continue
            ...
            if kp_score[9] > joint_threshold: 
                det_box[1] = min(det_box[1], int(mkp[9][1] - 20))
            if kp_score[10] > joint_threshold: 
                det_box[1] = min(det_box[1], int(mkp[10][1] - 20))
            ...
            for idx in range(17):
                ...
                if idx <= 12 and depth_map is not None:
                    ...
                    if z > 0: 
                        ground_h = 1200 - ((py - cy) * z / fy)
                        X = (px - cx) * z / fx
                        Y = (py - cy) * z / fy
                        Z = z
                kp_3d.append([px, py, z, kp_score[idx], ground_h, X, Y, Z])
            all_students_data.append({"id": i, "box": det_box, "keypoints": kp_3d, "name": f"Student {i+1}", "score": float(score), "raw_mkp": mkp, "raw_ks": kp_score})

        processed = self.action_monitor.get_classroom_actions(all_students_data) if (self.logic_initialized and self.action_monitor) else []

        # --- INSTANT CO-LOCATION TRACKER ---
        matched_track_ids = set()
        
        for st in processed:
            xb = st['box']
            cx_val = int((xb[0] + xb[2]) / 2)
            cy_val = int((xb[1] + xb[3]) / 2)
            new_centroid = (cx_val, cy_val)
            posture = st['posture']
            action = st['action']
            score = st['score']
            ...
            for tid, track_info in self.tracks.items():
                dist = np.linalg.norm(np.array(new_centroid) - np.array(track_info['centroid']))
                if dist < min_dist:
                    min_dist = dist
                    best_id = tid
        ...
        for tid, track in self.tracks.items():
            xb = [int(x) for x in track['box']]
            posture = track['posture']
            action = track['action']
            score = track['score']
            col = track['color']
            name = track['name']
            mkp_s, ks_s = track['raw_mkp'], track['raw_ks']

            # FACE BLUR
            head_indices = [0, 1, 2, 3, 4]
            visible_head_pts = [mkp_s[i] for i in head_indices if ks_s[i] > 0.35]
            
            if len(visible_head_pts) >= 2:
                pts = np.array(visible_head_pts)
                hx1, hy1 = np.min(pts, axis=0)
                hx2, hy2 = np.max(pts, axis=0)
                
                fw = hx2 - hx1
                box_w = xb[2] - xb[0]
                fw = max(fw, box_w * 0.38)
                fh = int(fw * 1.55)
                ...
                pad_top = int(fw * 0.82)     
                pad_bottom = int(fw * 0.90)  
                pad_side = int(fw * 0.48)    
                
                bx1 = max(0, int(center_x - pad_side))
                bx2 = min(orig_w - 1, int(center_x + pad_side))
                by1 = max(0, int(center_y - pad_top))
                by2 = min(orig_h - 1, int(center_y + pad_bottom))
                ...
                if bx2 > bx1 and by2 > by1:
                    ...
                    if roi_h > 30 and roi_w > 30:
                        small_roi = cv2.resize(face_roi, (int(roi_w * 0.25), int(roi_h * 0.25)), interpolation=cv2.INTER_LINEAR)
            ...
            # HUD TEXT
            hud_text = f"{name} | {posture} | {action} | {score:.2f}"
            text_size = cv2.getTextSize(hud_text, 0, 0.6, 2)[0]
            cv2.rectangle(display_image, (xb[0], xb[1]-35), (xb[0] + text_size[0] + 20, xb[1]), col, -1)
            cv2.putText(display_image, hud_text, (xb[0]+10, xb[1]-10), 0, 0.6, (255,255,255), 2)
            ...
            state_key = (posture, action)
            census_counts[state_key] = census_counts.get(state_key, 0) + 1

        # --- VOICE REPORT ---
        btn = self.snap_button.is_pressed if (self.gpio_ready and self.snap_button) else False
        if key_pressed == ord('s') or btn:
            print("\n🔊 Spoken Census Request Received!")
            
            sentences = []
            
            if len(census_counts) > 2:
                print("   [Summarization Active] Compressing phrase length...")
                for (posture, action), count in census_counts.items():
                    p_lower = posture.lower()
                    if action == "ATTENTIVE":
                        continue  
                    elif action == "RAISING HAND":
                        sentences.append(f"{count} {p_lower} raising hand")
                    elif action == "LOOKING AWAY":
                        sentences.append(f"{count} {p_lower} looking away")
                    elif action == "PRAYING":
                        sentences.append(f"{count} {p_lower} praying")
                
                if len(sentences) == 0:
                    full_t = "All students are sitting attentively."
                else:
                    full_t = "Classroom census: " + ", ".join(sentences[:-1]) + (", and " if len(sentences) > 1 else "") + sentences[-1] + ", with the rest attentive."
            else:
                for (posture, action), count in census_counts.items():
                    is_are = "is" if count == 1 else "are"
                    student_s = "student" if count == 1 else "students"
                    hand_s = "hand" if count == 1 else "hands"
                    p_lower = posture.lower()
                    
                    if action == "ATTENTIVE":
                        sentences.append(f"{count} {student_s} {is_are} {p_lower} attentively")
                    elif action == "RAISING HAND":
                        sentences.append(f"{count} {student_s} {is_are} {p_lower} and raising their {hand_s}")
                    elif action == "LOOKING AWAY":
                        sentences.append(f"{count} {student_s} {is_are} {p_lower} and looking away")
                    elif action == "PRAYING":
                        sentences.append(f"{count} {student_s} {is_are} {p_lower} and praying")

                if len(census_counts) == 0:
                    full_t = "No students detected on screen."
                else:
                    if len(sentences) == 1:
                        full_t = sentences[0] + "."
                    else:
                        full_t = ", ".join(sentences[:-1]) + ", and " + sentences[-1] + "."
            
            full_t = full_t.capitalize()
            print(f"🗣️ Generated Phrase: {full_t}")
