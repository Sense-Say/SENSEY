    ...
    def visualize_pose_estimation_result(self, results, image, model_height, model_width, depth_map=None, intrinsics=None, vpu_faces=None, detection_threshold=0.4, joint_threshold=0.4, key_pressed=255):
        ...
        if not self.logic_initialized:
            try:
                ...
                if os.path.exists("/home/raspberrypi/Documents/just_scanned.flag"):
                    self.report_printed = False; os.remove("/home/raspberrypi/Documents/just_scanned.flag")
        ...
        if key_pressed == ord('s') or btn:
            ...
            for i, (box, score) in enumerate(zip(b_d, s_d)):
                if float(score) > detection_threshold:
                    db = self.map_box_to_original_coords(box, display_image.shape[1], display_image.shape[0], model_width, model_height)
            ...
            return "TRIGGERED"
        if key_pressed == ord('q'): return "QUIT"
       ...
       # 4. 3D FUSION
        ...
        for i, (box, score, kp, kp_score) in enumerate(zip(b_d, s_d, k_d, ks_d)):
            ...
            det_box = self.map_box_to_original_coords(box, display_image.shape[1], display_image.shape[0], model_width, model_height)
            mkp = self.map_keypoints_to_original_coords(kp.reshape(17, 2), display_image.shape[1], display_image.shape[0], model_width, model_height)
            
            # Expand box for hands
        ...
        # --- 8. SMART OFFLINE AUDIO REPORT ---
        if not self.report_printed and self.warmup_frames > 25:
            full_t = ""
            if len(action_groups) == 0: 
                full_t = "No students detected."
            else:
                print("\n📊 CLASSROOM STATUS REPORT:")
                for action, names_list in action_groups.items():
                    real_n = [n for n in names_list if "Student" not in n]
                    unkn_c = sum(1 for n in names_list if "Student" in n)
                    
                    parts = []
                    if real_n: parts.append(", ".join(real_n))
                    if unkn_c == 1: parts.append("1 Student")
                    elif unkn_c > 1: parts.append(f"{unkn_c} Students")
                    
                    subject = " and ".join(parts)
                    verb = "is" if (len(real_n) + unkn_c) == 1 else "are"
                    
                    # 🚀 SMART PHRASING: Add "conducting" for Anthem and Oath
                    if action in ["NATIONAL ANTHEM", "PATRIOTIC OATH"]:
                        action_phrase = f"conducting {action}"
                    else:
                        action_phrase = action
                        
                    line = f"{subject} {verb} {action_phrase}."
                    print(f"   👉 {line}")
                    full_t += line + " "
            ...
            if PIPER_READY:
                def speak(speech_text):
                    ...
                    try:
                        # Piper @ 0.85 Speed
                        cmd = f'echo "{speech_text}" | {PIPER_EXE} --model {PIPER_MODEL} --length_scale 0.85 --output-raw | aplay -r 22050 -f S16_LE -t raw'
            ...
            if PIPER_READY:
                def speak(speech_text):
                    import subprocess
                    try: subprocess.run(f'echo "{speech_text}" | {PIPER_EXE} --model {PIPER_MODEL} --length_scale 1.3 --output-raw | aplay -r 22050 -f S16_LE -t raw', shell=True)
                    except: pass
                threading.Thread(target=speak, args=(full_t,), daemon=True).start()
