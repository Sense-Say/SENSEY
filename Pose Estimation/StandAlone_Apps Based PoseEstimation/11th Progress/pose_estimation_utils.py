...
class PoseEstPostProcessing:
    ...
    def map_box_to_original_coords(self, box, orig_w, orig_h, model_w, model_h):
        return [box[0] * (orig_w/model_w), box[1] * (orig_h/model_h), box[2] * (orig_w/model_w), box[3] * (orig_h/model_h)]
    ...
    def map_keypoints_to_original_coords(self, keypoints, orig_w, orig_h, model_w, model_h):
        ...
        mapped = np.zeros_like(keypoints); mapped[:, 0], mapped[:, 1] = keypoints[:, 0] * scale_x, keypoints[:, 1] * scale_y
        return mapped
    ...
    def visualize_pose_estimation_result(self, results, image, model_height, model_width, depth_map=None, intrinsics=None, vpu_faces=None, detection_threshold=0.4, joint_threshold=0.4, key_pressed=255):
        ...
        if not self.logic_initialized:
            try:
                ...
                if GPIO_AVAILABLE:
                    try: self.snap_button = Button(26, pull_up=True, bounce_time=0.1)
        ...
        # --- 🚀 4. FULL 3D KEYPOINT FUSION (UPPER BODY ONLY) ---
        ...
        for i, (box, score, kp, kp_score) in enumerate(zip(b_d, s_d, k_d, ks_d)):
            ...
            kp_3d = []
            # We process all 17 joints, but only calculate Z for the first 13 (Head to Hips)
            for idx in range(17):
                px, py = int(mkp[idx][0]), int(mkp[idx][1])
                z, ground_h = 0, 0
                
                # Only calculate depth for Upper Body (0-12) to save resources/avoid occlusion
        ...
        for st in processed:
            ...
            # Visual Label
            cv2.putText(display_image, f"{name} | {act} | {dist:.1f}m", (xb[0]+5, xb[1]-7), 1, 0.5, (255,255,255), 1)
            
            # Skeleton (Drawing lines based on confidence)
            ...
            # 🚀 Audio Grouping (NAMES ONLY, No distance)
            if act not in action_groups: action_groups[act] = []
            action_groups[act].append(name)

        # --- 6. SMART AUDIO REPORT ---
        if not self.report_printed and self.warmup_frames > 20:
            full_t = ""
            ...
            if PIPER_READY:
                def speak(txt):
                    import subprocess
                    try: subprocess.run(f'echo "{txt}" | {PIPER_EXE} --model {PIPER_MODEL} --length_scale 0.85 --output-raw | aplay -r 22050 -f S16_LE -t raw', shell=True)
                    except: pass
