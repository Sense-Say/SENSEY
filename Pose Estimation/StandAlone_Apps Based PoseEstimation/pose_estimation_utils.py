...
class PoseEstPostProcessing:
    ...
    def visualize_pose_estimation_result(
        ...
        # Dictionary to group names by action: { "Raising Hand": ["Edward", "Michael"], ... }
        action_groups = {} 
        ...
        # --- 6. PRINT GROUPED REPORT (Once after startup) ---
        if not self.report_printed and len(action_groups) > 0:
            print("\n📊 CLASSROOM STATUS REPORT:")
            
            # Iterate through the groups and print in the requested format
            for action, names in action_groups.items():
                # Join names with commas (e.g., "Edward, Michael")
                names_string = ", ".join(names)
                print(f"   👉 {names_string} is {action}.")
