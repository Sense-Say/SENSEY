...
class PoseEstPostProcessing:
    ...
    def visualize_pose_estimation_result(
        ...
        # 🚀 COLOR FIX: Convert RGB (from OAK-D/Hailo) back to BGR for OpenCV Display
        ...
        # --- 3. TRIGGER HANDLING (S Key or Button) ---
        key = cv2.waitKey(1) & 0xFF
        
        # Safe button check
        btn_pressed = False
        if self.gpio_ready and self.snap_button is not None:
            btn_pressed = self.snap_button.is_pressed

        if key == ord('s') or btn_pressed:
            # Save Screenshot (display_image is already BGR)
            ...
            print("🛑 SNAP TRIGGERED: Exiting for Recognition...")
            try: os._exit(0)
            except: sys.exit(0)

        if key == ord('q'): os._exit(0)

        # --- 4. 3D FUSION & DATA COLLECTION ---
        all_students_data = []
        for i, (box, score, kp, kp_score) in enumerate(zip(b_d, s_d, k_d, ks_d)):
            ...
            for idx in range(17):
                ...
                if depth_map is not None:
                    ...
                    if z > 0:
                        y_world_rel = (py - cy) * z / fy
                        ground_h = 1200 - y_world_rel # 1.2m Teacher Baseline
        ...
        # --- 6. DRAWING VPU FACES (SCRFD) ---
        if vpu_faces:
            for face in vpu_faces:
                fx1, fy1 = int(face.xmin * orig_w), int(face.ymin * orig_h)
                fx2, fy2 = int(face.xmax * orig_w), int(face.ymax * orig_h)
                cv2.rectangle(display_image, (fx1, fy1), (fx2, fy2), (255, 255, 255), 1)

        # --- 7. DRAWING & GROUPING ---
        action_groups = {} 
        for st in processed_students:
            xb = [int(x) for x in st['box']]
            act, col = st['action'], st['color']
            dist_m = st['keypoints'][0][2] / 1000.0 # Nose dist

            # Draw Label
            lbl = f"{st['name']} | {act} | {dist_m:.1f}m"
            cv2.rectangle(display_image, (xb[0], xb[1]), (xb[2], xb[3]), col, 2)
            cv2.rectangle(display_image, (xb[0], xb[1]-25), (xb[0]+320, xb[1]), col, -1)
            cv2.putText(display_image, lbl, (xb[0]+5, xb[1]-7), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1)

            # Draw Skeleton
            mkp, ks = st['raw_mkp'], st['raw_ks']
            for j0, j1 in JOINT_PAIRS:
                if ks[j0] > joint_threshold and ks[j1] > joint_threshold:
                    cv2.line(display_image, (int(mkp[j0][0]), int(mkp[j0][1])), (int(mkp[j1][0]), int(mkp[j1][1])), (0, 255, 255), 2)

            # Audio Grouping
            n_aud = f"{st['name']} at {dist_m:.1f} meters" if dist_m > 0 else st['name']
            if act not in action_groups: action_groups[act] = []
            action_groups[act].append(n_aud)
        ...
        if not self.report_printed and len(action_groups) > 0:
            ...
            for action, n_list in action_groups.items():
                ...
                subj = " and ".join(parts) if parts else "No one"
                verb = "is" if (len(real) + unkn) == 1 else "are"
                line = f"{subj} {verb} {action}."
                print(f"   👉 {line}")
                full_text += line + " "
        ...
        self.prev_time = time.time()
        cv2.putText(display_image, f"FPS: {fps:.1f}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
