...
class PoseEstPostProcessing:
    ...
    def visualize_pose_estimation_result(
            ...
            model_height: int,
            model_width: int,
            depth_map: np.ndarray = None, # Passed from OAK-D Lite
        ...
        if not hasattr(self, 'logic_initialized'):
            ...
            try:
                ...
                print("⚠️ Action Logic Not Found. Using Dummy.")
            
            # Init Button on GPIO 26
            if GPIO_AVAILABLE:
                try:
        ...
        # --- 2. DATA EXTRACTION ---
        try:
            if 'predictions' in results: b_data, s_data, k_data, ks_data = results['predictions']
            elif 'bboxes' in results: b_data, s_data, k_data, ks_data = results['bboxes'][0], results['scores'][0], results['keypoints'][0], results['joint_scores'][0]
            else: return image
        except Exception: return image

        orig_h, orig_w = image.shape[:2]

        # --- 3. INPUT HANDLING (Button or Key) ---
        key = cv2.waitKey(1) & 0xFF
        button_pressed = self.snap_button.is_pressed if self.gpio_ready else False

        if key == ord('s') or button_pressed:
            save_image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            cv2.imwrite("/home/raspberrypi/Documents/temp_screenshot.jpg", save_image)
            
            pose_boxes = []
            for i, (box, score) in enumerate(zip(b_data, s_data)):
                if float(score) > detection_threshold:
                    det_box = self.map_box_to_original_coords(box, orig_w, orig_h, model_width, model_height)
                    pose_boxes.append({"id": str(i), "box": [int(x) for x in det_box]})
            
            with open("/home/raspberrypi/Documents/temp_boxes.json", "w") as f: json.dump(pose_boxes, f)
            with open("/home/raspberrypi/Documents/trigger.txt", "w") as f: f.write("snap")
            
            print("🛑 TRIGGER: Exiting for Face Scan...")
            try: os._exit(0)
            except: sys.exit(0)
        ...
        # --- 4. DATA COLLECTION & 3D FUSION ---
        all_students_data = []

        for i, (box, score, kp, kp_score) in enumerate(zip(b_data, s_data, k_data, ks_data)):
            if float(score) < detection_threshold: continue
            
            det_box = self.map_box_to_original_coords(box, orig_w, orig_h, model_width, model_height)
            mapped_kp = self.map_keypoints_to_original_coords(kp.reshape(17, 2), orig_w, orig_h, model_width, model_height)

            # Get Z for every joint to feed the 3D Action Logic
            kp_3d = []
            for idx in range(17):
                px, py = int(mapped_kp[idx][0]), int(mapped_kp[idx][1])
                z = 0
                if depth_map is not None:
                    # Median filter to avoid black pixel noise
                    roi = depth_map[max(0,py-2):py+3, max(0,px-2):px+3]
                    z = np.median(roi) if roi.size > 0 else 0
                kp_3d.append([px, py, z, kp_score[idx]])
            ...
            all_students_data.append({
                ...
                "keypoints": kp_3d,
                "name": name,
                "raw_mapped_kp": mapped_kp, # Passed for drawing skeleton later
                "raw_kp_score": kp_score
            })

        # --- 5. ACTION LOGIC EXECUTION ---
        # We pass the full room data so the logic can check distances between students
        processed_students = self.action_monitor.get_classroom_actions(all_students_data)

        # --- 6. DRAWING & REPORTING LOOP ---
        action_groups = {} 

        for student in processed_students:
            xmin, ymin, xmax, ymax = [int(x) for x in student['box']]
            name = student['name']
            act = student['action']
            col = student['color']
            dist_m = student['keypoints'][0][2] / 1000.0 # Distance of Nose

            # Audio Grouping
            audio_name = f"{name} at {dist_m:.1f} meters" if dist_m > 0 else name
            if act not in action_groups: action_groups[act] = []
            action_groups[act].append(audio_name)

            # Draw UI
            dist_label = f"{dist_m:.1f}m" if dist_m > 0 else ""
            label_text = f"{name} | {act} | {dist_label}"
            
            cv2.rectangle(image, (xmin, ymin), (xmax, ymax), col, 2)
            cv2.rectangle(image, (xmin, ymin-25), (xmin+300, ymin), col, -1)
            cv2.putText(image, label_text, (xmin+5, ymin-7), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1)

            # Skeleton
            mkp = student['raw_mapped_kp']
            ks = student['raw_kp_score']
            for j, s in zip(mkp, ks):
                if s > joint_threshold: cv2.circle(image, (int(j[0]), int(j[1])), 2, (255, 0, 255), -1)
            for j0, j1 in JOINT_PAIRS:
                if ks[j0] > joint_threshold and ks[j1] > joint_threshold:
                    cv2.line(image, (int(mkp[j0][0]), int(mkp[j0][1])), (int(mkp[j1][0]), int(mkp[j1][1])), (0, 255, 255), 2)

        # --- 7. SMART AUDIO REPORTING ---
        if not self.report_printed and len(action_groups) > 0:
            full_speech_text = ""
            print("\n📊 CLASSROOM STATUS REPORT:")
            for action, names_list in action_groups.items():
                
                real_names = [n for n in names_list if "Student" not in n]
                student_count = sum(1 for n in names_list if "Student" in n)
                
                parts = []
                if real_names: parts.append(", ".join(real_names))
                if student_count == 1: parts.append("1 Student")
                elif student_count > 1: parts.append(f"{student_count} Students")
                
                subject = " and ".join(parts) if len(parts) > 0 else "No one"
                verb = "is" if (len(real_names) + student_count) == 1 else "are"
                line = f"{subject} {verb} {action}."
                
                print(f"   👉 {line}")
                full_speech_text += line + " "
            ...
            if AUDIO_ENABLED:
                def speak():
                    try:
                        tts = gTTS(text=full_speech_text, lang='en', slow=False)
                        tts.save("/home/raspberrypi/Documents/report.mp3")
                        print("🔊 Speaking...")
                        playsound("/home/raspberrypi/Documents/report.mp3")
                        if os.path.exists("/home/raspberrypi/Documents/report.mp3"): 
