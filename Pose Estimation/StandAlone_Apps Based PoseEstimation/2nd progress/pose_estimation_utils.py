...
class PoseEstPostProcessing:
    ...
    def visualize_pose_estimation_result(
        ...
        # --- 1. FAIL-SAFE INITIALIZATION (Runs once) ---
        if not hasattr(self, 'logic_initialized'):
            # Initialize all required variables for fail-safe operation
            self.npu_face_ready = False # Set to False permanently
            self.prev_time = time.time()
            
            # Action Logic (Always loads a monitor, even a dummy one)
            try:
                sys.path.append("/home/raspberrypi/Downloads")
                from action_logic import StudentActionMonitor
                self.action_monitor = StudentActionMonitor()
                print("✅ Action Logic Loaded.")
            except:
                class DummyMonitor:
                    def get_action(self, kp, idx, center): return "Monitoring", (0, 255, 0)
                self.action_monitor = DummyMonitor()
                print("⚠️ Action Logic Not Found. Using Dummy Monitor.")
            
            self.logic_initialized = True

        # 2. Dynamic Label Mapping: Read JSON on every frame for instant updates
        self.name_map = {} # Reset map
        map_path = "/home/raspberrypi/Downloads/name_map.json"
        if os.path.exists(map_path):
            try:
                with open(map_path, "r") as f:
                    self.name_map = json.load(f)
            except:
                pass # Fail silently if the JSON file is corrupt
        ...
        # 5. DATA EXTRACTION FIX (CRITICAL)
        ...
        try:
            ...
            return image # Safely return the image if data is incomplete
            
        orig_h, orig_w = image.shape[:2]

        # --- 6. MAIN LOOP ---
        for i, (det_box, det_score, det_kp, det_kp_score) in enumerate(zip(box_data, score_data, kp_data, kp_score_data)):
            
            score_val = float(det_score)
            if score_val < detection_threshold: continue
            
            det_box = self.map_box_to_original_coords(det_box, orig_w, orig_h, model_width, model_height)
            xmin, ymin, xmax, ymax = [int(x) for x in det_box]

            # A. ACTION LOGIC
            raw_kp = det_kp.reshape(17, 2)
            formatted_kp = [[p[0], p[1], s] for p, s in zip(raw_kp, det_kp_score)]
            center = (xmin + (xmax-xmin)//2, ymin + (ymax-ymin)//2)
            act_txt, act_col = self.action_monitor.get_action(formatted_kp, i, center)

            # B. DYNAMIC LABEL MAPPING (The Final Goal)
            # This is the line that makes your whole project work:
            student_name = self.name_map.get(str(i), f"Student {i+1}") 

            # C. DRAWING
            display_text = f"{student_name} | {act_txt} | {score_val:.2f}"
            
            cv2.rectangle(image, (xmin, ymin), (xmax, ymax), act_col, 2)
            cv2.rectangle(image, (xmin, ymin-25), (xmin + 260, ymin), act_col, -1)
            cv2.putText(image, display_text, (xmin + 5, ymin - 7), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

            # D. SKELETON (Original)
            mapped_kp_draw = self.map_keypoints_to_original_coords(raw_kp, orig_w, orig_h, model_width, model_height)
            for j, s in zip(mapped_kp_draw, det_kp_score):
                if s > joint_threshold: cv2.circle(image, (int(j[0]), int(j[1])), 2, (255, 0, 255), -1)
            for j0, j1 in JOINT_PAIRS:
                if det_kp_score[j0] > joint_threshold and det_kp_score[j1] > joint_threshold:
                    cv2.line(image, (int(mapped_kp_draw[j0][0]), int(mapped_kp_draw[j0][1])), 
                             (int(mapped_kp_draw[j1][0]), int(mapped_kp_draw[j1][1])), (0, 255, 255), 2)
