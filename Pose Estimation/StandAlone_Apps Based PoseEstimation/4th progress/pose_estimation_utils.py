...
class PoseEstPostProcessing:
    ...
    def visualize_pose_estimation_result(
        ...
        if key == ord('s'):
            ...
            print("🛑 TRIGGER: Exiting Pose to run Face Scan (Spatial Mode)...")
            try: os._exit(0)
            except: sys.exit(0)
        ...
        # --- 5. MAIN DRAWING & DATA COLLECTION LOOP ---
        action_groups = {} 
        ...
        # --- 6. PRINT & SPEAK GROUPED REPORT (Runs once after startup) ---
            ...
            if AUDIO_ENABLED:
                def speak_report():
                    try:
                        ...
                        tts.save(audio_file)
                        # Playsound inside thread so it doesn't freeze video
                        print("🔊 Speaking Report...")
