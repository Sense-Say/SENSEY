...
def handle_voice_command(cmd):
    ...
    # ---------------- THREAD SAFE 5S WAV NOTES ("The Sponge") ---------------- #
        ...
        if "yes" in cmd or "correct" in cmd:
            ...
            # 🚀 FIX: Using paplay
            subprocess.run(f'echo "Start" | {PIPER_EXE} --model {PIPER_MODEL} --output_raw | paplay --raw --format=s16le --rate=22050 --channels=1', shell=True) 
            ...
            voice_note_buffer = [] 
            is_recording_note = True # Turn on the sponge
            
            # 🚀 GUARANTEED TIMER: Wait exactly 5 seconds
            time.sleep(5.0)
            
            is_recording_note = False # Turn off the sponge
            ...
            if len(voice_note_buffer) > 0:
                ...
                print("🔴 Buffer was empty! Mic might be locked.")
                    
            print("✅ Recording finished. Saving...")
            subprocess.run(f'echo "Voice note saved. Continue recording." | {PIPER_EXE} --model {PIPER_MODEL} --output_raw | paplay --raw --format=s16le --rate=22050 --channels=1', shell=True)
