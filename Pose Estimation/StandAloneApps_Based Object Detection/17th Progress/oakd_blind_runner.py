...
try:
    start_beep_sound = pygame.mixer.Sound("/home/raspberrypi/Downloads/recording_notes.wav")
    arrival_sound = pygame.mixer.Sound("/home/raspberrypi/Downloads/arrived_destination.wav")
...
# ... inside get_instruction ...
        if self.distance_to_wp < 0.45:
            # 1. Queue the "Reached Point" message
            audio_queue.put({"type": "text", "msg": f"Reached {target['label']}."})
...
def handle_voice_command(cmd):
    ...
    # 🚀 REVISED: RUNS SAFELY IN BACKGROUND THREAD WITH PYGAME SOUNDS
        ...
        if "yes" in cmd or "correct" in cmd:
            ...
            # 2. 🔔 Play Custom Start Beep (via Queue)
            audio_queue.put({"type": "beep"})
            time.sleep(2.0) # Wait for beep
            
            # 3. 🎙️ ACTIVATE THE SPONGE
            ...
            voice_note_buffer = [] # Empty the sponge
            note_recording_start_time = time.time()
            is_recording_note = True # Turns on the callback recorder
            
            # ⏳ Wait for the callback to finish filling the sponge (5 seconds)
            while is_recording_note:
                time.sleep(0.1) # Sleep main thread while callback works
            
            # 4. 🔔 Play Custom End Beep (via Queue)
            audio_queue.put({"type": "beep"})
            time.sleep(1.0)
            
            # 5. 💾 SQUEEZE THE SPONGE TO A FILE
            if len(voice_note_buffer) > 0:
                try:
                    # Flatten the list of arrays into one big audio array
                    audio_data = np.concatenate(voice_note_buffer, axis=0)
                    wav.write(note_path, native_rate, audio_data)
                    print(f"✅ Saved {len(voice_note_buffer)} chunks to {note_path}")
...
def run():
    ...
    def audio_callback(indata, frames, time_info, status):
        ...
        # 🚀 1. THE SPONGE LOGIC (Captures Voice Note)
        if is_recording_note:
            # We need raw int16 for the .wav file
            ...
            if time.time() - note_recording_start_time >= 5.0:
                is_recording_note = False # Stop the sponge
            return # 🛑 Exit early so Vosk doesn't hear the note
            
        # 🚀 2. STANDARD VOSK LOGIC
