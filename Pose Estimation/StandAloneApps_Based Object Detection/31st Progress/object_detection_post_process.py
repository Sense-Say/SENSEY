...
def draw_detections(detections: dict, img_out: np.ndarray, labels, vio_data=None, target_yaw=None, target_dist=None, depth_frame=None, state_text="IDLE"):
    ...
    if vio_data:
        ...
        # 🚀 NORMALIZATION: Ensure the compass display is always 0-360
        hud_yaw = yaw % 360
        
        # Compass Bar (Top)
        cv2.rectangle(img_out, (0, 0), (width, 70), (0, 0, 0), -1)
        cv2.line(img_out, (center_x, 10), (center_x, 60), (0, 255, 255), 2) # Center Notch

        pixels_per_degree = width / 90 
        
        # 2. Draw Sliding Tape
        # We iterate relative to the raw 'yaw' to keep the sliding smooth
        for deg in range(int(yaw - 45), int(yaw + 45)):
            ...
            if 0 < screen_x < width:
                if deg % 15 == 0:
                    ...
                    # Show the human-readable 0-359 degree number
                    cv2.putText(img_out, str(deg % 360), (screen_x - 10, 60), 0, 0.4, (255, 255, 255), 1)

        # 3. 🚀 THE FIX: PINNED GREEN ARROW
        if target_yaw is not None:
            # We calculate the relative offset using the Wrap-Around formula
            # This ensures the arrow stays 'stuck' to the specific degree on the tape
            relative_angle = (target_yaw - yaw + 180) % 360 - 180
            arrow_x = center_x + int(relative_angle * pixels_per_degree)
            
            if 0 < arrow_x < width:
                # Color logic: Green if in center path, else Gray
                ...
                # Draw distance remaining to THIS point
                if target_dist is not None:
                    cv2.putText(img_out, f"{target_dist:.2f}m", (arrow_x - 20, 35), 0, 0.5, color, 2)

        # 4. DASHBOARD: Show Pitch and Roll
    ...
    # 5. Object Detection Loop (Retained exactly as original)
