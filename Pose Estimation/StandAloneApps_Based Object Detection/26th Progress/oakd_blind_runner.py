...
class NavigationManager:
    ...
    def load_path(self, path_data):
        """🚀 KEEPS ALL BREADCRUMBS FOR 100% ACCURATE DISTANCE"""
        self.path = []
        for p in path_data:
            label = str(p[2]).lower()
            ...
            self.path.append({
                ...
                "total_dist": p[5] if len(p) > 5 else 0.0 # 🚀 READ THE NEW 6TH ELEMENT
            })
    ...
    def get_instruction(self, cur_x, cur_z, cur_yaw, is_on_demand=False):
        ...
        if is_on_demand:
            ...
            if turn_err > 30: direction = "Turn right"
            elif turn_err < -30: direction = "Turn left"
            return f"{direction} and walk {total_dist:.2f} meters."
        ...
        if self.distance_to_wp < 0.45:
            ...
            if node_type == "ANCHOR" or note:
                ...
                audio_queue.put({"type": "text", "msg": f"{direction} and walk {next_dist:.2f} meters."})
                ...
                while True:
                   ...
                   # 🚀 5. THE NOISE-CANCELLING PEDOMETER
                            ...
                            if 0 <= dy < 1008 and 0 <= dx < 1344:
                                ...
                                if 0.5 < z < 8.0:
                                    if f.id in feat_history:
                                        # 🚀 FIX: Pure Z-Depth tracking only
                                        # We only care about objects getting closer/further.
                    ...
                    if warmup_frames < 30:
                        ...
                        # 🚀 THE FIX: Forward Vector Only
                        # By calculating X and Z strictly based on the actual_step (which is purely depth change)
                        # and multiplying by the Sine/Cosine of the Yaw, we eliminate the "Camera Swing" error.
                        # If you are turning (is_rotating), actual_step is 0, so X and Z stay perfectly frozen!
                    ...
                    if STATE == "RECORDING":
                        if len(recorded_path) > 0:
                            ...
                            if dist_from_last > 0.5 or yaw_change > 20:
                                # 🚀 THE FIX: We now save the 'total_dist' as the 6th element
                                recorded_path.append([current_x, current_z, "path", current_yaw, "", total_dist])
