...
class NavigationManager:
    ...
    def get_instruction(self, cur_x, cur_z, cur_yaw):
        ...
        if self.distance_to_wp < 0.5:
            # 🚀 SEQUENCE: Arrival -> Note -> Next Instruction
            tasks = []
            tasks.append({"type": "text", "msg": f"Reached {target['label']}."})
            
            if target['note'] and target['note'].endswith(".wav"):
                tasks.append({"type": "wav", "path": os.path.join(DOC_PATH, target['note'])})
            
            self.current_wp_index += 1
            if self.current_wp_index < len(self.path):
                # Calculate turn
                next_t = self.path[self.current_wp_index]
                next_turn = (math.degrees(math.atan2(next_t['x']-target['x'], next_t['z']-target['z'])) - cur_yaw + 180)%360 - 180
                
                direction = "Continue straight."
                if next_turn > 30: direction = "Turn right."
                elif next_turn < -30: direction = "Turn left."
                
                tasks.append({"type": "text", "msg": direction})
            else:
                self.active = False
                tasks.append({"type": "text", "msg": "Arrived at destination."})
            
            for t in tasks: audio_queue.put(t) # 🚀 Queue the sequence!
            return None # We return None because the sequence is now in the queue
