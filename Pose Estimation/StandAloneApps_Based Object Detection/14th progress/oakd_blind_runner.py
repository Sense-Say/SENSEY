...
def run():
    ...
    last_imu_t = None
    feat_history = {} 
    motion_window = [] # 🚀 NEW: Tracks the last 10 frames to cancel out noise 
                ...
                while True:
                    ...
                    for data in imuData:
                        for packet in data.packets:
                            ...
                            if abs(accel_mag - 9.81) > 0.3: # Less strict step detection
                                is_stepping = True
                    ...
                    # 🚀 3. RUN AI FIRST (To get masking boxes)
                    ...
                    for f in fea_data:
                        ...
                        # 🟢 VISUALIZER: Draw ALL tracked features as small YELLOW dots
                        cv2.circle(rgb_isp, (dx, dy), 2, (0, 255, 255), -1)
                        
                        if 0 <= dy < 1008 and 0 <= dx < 1344:
                            # 🛑 MASKING: Ignore points on the detected laptop, person, etc.
                            ...
                            if 0.5 < z < 8.0:
                                if f.id in feat_history:
                                    ...
                                    # 🚀 CRITICAL FIX: Collect ALL noise (positive and negative) under 40cm
                                    if abs(d_z) < 0.40:
                                        deltas.append(d_z)
                    ...
                    # Calculate the net movement of THIS single frame
                    frame_move = sum(deltas) / len(deltas) if deltas else 0.0
                    
                    # 🚀 Add to smoothing window (last 10 frames)
                    motion_window.append(frame_move)
                    if len(motion_window) > 10: motion_window.pop(0)

                    # 🚀 THE "ANTI-CHRISTMAS LIGHT" MATH
                    # By averaging the window, the positive and negative shimmers cancel each other out to 0.0
                    smooth_move = sum(motion_window) / len(motion_window)

                    # 🚀 THE "FORWARD ONLY" GATE
                    # We only add distance if the smoothed net movement is definitely pushing forward (> 0.5cm per frame)
                    if smooth_move > 0.005: 
                        total_dist += smooth_move
                        rad_yaw = math.radians(current_yaw)
                        current_x = total_dist * math.sin(rad_yaw)
                        current_z = total_dist * math.cos(rad_yaw)
                        
                        # 🔴 VISUALIZER: Show a clear "WALKING" text instead of confusing blinking dots
