...
def draw_detections(detections: dict, img_out: np.ndarray, labels, vio_data=None, target_yaw=None, target_dist=None, depth_frame=None, state_text="IDLE"):
    ...
    if vio_data:
        ...
        # 2. Pinned Green Arrow (Goal) with Distance
        if target_yaw is not None:
            ...
            if 0 < arrow_x < width:
                ...
                if target_dist is not None:
                    cv2.putText(img_out, f"{target_dist:.1f}m", (arrow_x - 15, 35), 0, 0.5, color, 2)

        # Dashboard
        cv2.putText(img_out, f"MODE: {state_text} | YAW: {int(yaw)} | DIST: {dist_total:.1f}m", (20, height-20), 0, 0.6, (255, 255, 255), 2)
