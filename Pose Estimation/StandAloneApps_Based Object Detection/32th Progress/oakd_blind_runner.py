...
def run():
    ...
    TICK_THRESHOLD_STOP = 3   # Frames of blockage needed to STOP ticking (faster for safety)
        ...
        # --- REVISED EXPLORATION SETTINGS ---
    # Lower this so the tick stays active even when closer to walls
    CENTER_CLEAR_THRESHOLD = 0.8  # Meters: (Was likely 2.0 or 1.5)
    # Now, $1.3m$ will be considered "CLEAR" and the tick will play.

    # How far to "see" a side aisle opening
    SIDE_OPENING_THRESHOLD = 2.0  # Meters: (Reduced for classrooms)

    # Stability timing
    AISLE_CONFIRM_FRAMES = 10 
    TICK_THRESHOLD_START = 8      # Faster start
    TICK_THRESHOLD_STOP = 3       # Quick stop for safety
        # Wall & Dead End triggers
    WALL_DISTANCE_LIMIT = 0.8     # Only say "Wall" when within 80cm
    DEAD_END_LIMIT = 0.7          # Only say "Dead End" when within 70cm
            ...
            with InferVStreams(group, in_p, out_p) as pipe:
                ...
                while True:
                    ...
                    # 🚀 D. ENVIRONMENTAL GEOMETRY (Wall & Dead End)
                       # --- REVISED FLATNESS (Optional) ---
                    # If you want it to be more sensitive to walls:
                    is_flat = (abs(l_dist - c_dist) < 0.25) and (abs(r_dist - c_dist) < 0.25)
                    
                    # 2. UPDATED DETECTION TRIGGERS
                    # If depth is 0, it means it's CLOSER than 0.4m (Blind Spot)
                    is_dangerously_close = (c_dist < 0.45) or (c_dist == 0)

                    # Wall detection now only triggers at 0.8m (approx. 2.5 feet)
                    detected_wall = (c_dist < WALL_DISTANCE_LIMIT) and (c_dist > 0.1) and is_flat
                    
                    # Dead end triggers at 0.7m
                    detected_dead_end = (l_dist < DEAD_END_LIMIT) and (c_dist < DEAD_END_LIMIT) and (r_dist < DEAD_END_LIMIT) and not is_flat

                    # 3. ACCURACY PERSISTENCE
                    ...
                    # 4. STABILIZED VIRTUAL RAIL (The Tick)
                    # The Tick plays if Center is clear (> 0.8m) AND not a Wall/DeadEnd
                    is_path_physically_clear = (c_dist > CENTER_CLEAR_THRESHOLD) and not detected_wall and not detected_dead_end and not is_dangerously_close
                    ...
                    if aisle_cooldown > 0:
                        ...
                        if is_dangerously_close:
                            audio_queue.put({"type": "text", "msg": "Stop! Too close."})
                            aisle_cooldown = 60 # Short 4s cooldown
