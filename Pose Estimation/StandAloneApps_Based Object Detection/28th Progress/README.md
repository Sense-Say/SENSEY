...
4.  **Green Arrow:** The target waypoint marker, which is mathematically pinned to a specific World Yaw degree to guide the teacher along the recorded path.    "window", "stop", "navigate", "start", "yes", "no", "correct", "wrong", "update", 
...
def run():
    ...
    # 🚀 1. HAPTIC HARDWARE INITIALIZATION (GPIO PWM)
    # Define the pins for your ULN2803A setup
    try:
        ...
        # This is the variable the loop was missing:
        motor_map = {
            "L": [m2, m3],
            "C": [m4, m5],
            "R": [m6, m1]
        }
        print("✅ Haptic Motors initialized on GPIO 17,27,22,23,24,25")
    except Exception as e:
        print(f"⚠️ Haptic Initialization Error: {e}")
        motor_map = {"L":[], "C":[], "R":[]} # Fallback to prevent crash
            ...
            with InferVStreams(group, in_p, out_p) as pipe:
                ...
                while True:
                   ...
                   # 🚀 5. MOTION-FILTERED PEDOMETER (ALWAYS TRACKING)
                    ...
                    # 🚀 7. EMERGENCY OCCLUSION SHIELD & PULSED HAPTIC FEEDBACK
                    if STATE == "NAVIGATING":
                        path_blocked = False
                        critical_warning = ""
                        ...
                        # 🚀 PULSE TIMER LOGIC (2s ON, 1.5s OFF)
                        # Total cycle = 3.5 seconds
                        current_time = time.time()
                        cycle_position = current_time % 3.5 
                        is_pulse_on = cycle_position < 2.0 # True for the first 2 seconds
                        
                        intensities = {"L": 0.0, "C": 0.0, "R": 0.0}
                                ...
                                for det in class_list:
                                    if len(det) >= 5:
                                        ...
                                        if score > 0.45: 
                                            ...
                                            # --- HAPTIC INTENSITY CALCULATION ---
                                            h_int = 0.0
                                            if 0.1 < obj_z < 1.0:
                                                h_int = 0.40
                                            elif 1.0 <= obj_z <= 5.0:
                                                h_int = 0.7 * (1.0 - (obj_z - 1.0) / 4.0)
                                            
                                            zone_key = "C"
                                            if cx < 0.33: zone_key = "L"
                                            elif cx > 0.66: zone_key = "R"
                                            intensities[zone_key] = max(intensities[zone_key], h_int)
                        ...
                        if not path_blocked:
                            center_roi = depth_raw[450:550, 622:722]
                        ...
                        # 🚀 APPLY PULSED HAPTICS TO HARDWARE
                        for zone, val in intensities.items():
                            for motor in motor_map[zone]:
                                # 🚀 THE PULSE GATE:
                                # If we are in the 1.5s 'OFF' window, force motor to 0.0
                                # If we are in the 2.0s 'ON' window, use the calculated intensity
                                if is_pulse_on:
                                    motor.value = val
                                else:
                                    motor.value = 0.0
